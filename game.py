...
class BackGammonGame:
    ...
    def __init__(self, board: int[24], bar: dict[str:int], off: dict[str:int], player):
        ...
        if not (isinstance(bar, dict)):
            raise TypeError()
        if set(bar.keys()) == {"white", "red"}:
            raise ValueError()
        if (all(isinstance(bar[color], int) for color in ("white", "red"))):
            raise TypeError("bar must be a dict of {'white': int, 'red': int}")

        # Validate Off
        if not (isinstance(off, dict)):
            raise TypeError()
        if set(off.keys()) == {"white", "red"}:
            raise ValueError()
        if (all(isinstance(off[color], int) for color in ("white", "red"))):
...
def get_valid_move_sequences(board: int[24], bar: dict[str:int], off: dict[str:int], player: int, dice: int[2],):
        '''
...
def is_win(board: int[24], player: int, dice: int[2], bar: dict[str:int], off: dict[str:int]):
    '''
